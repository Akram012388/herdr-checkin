...
pub(crate) struct QueueEntry {
    ...
    /// Falls back to the public `t{N}` segment from `tab_id` in the row. Migration-safe.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub(crate) tab_label: Option<String>,
    /// The pane's manual label (e.g. `orchestrator`), resolved from `pane list`; usually absent, in
    /// which case the row shows Herdr's stable public pane allocation number (`pA` -> `pane 10`).
    /// Migration-safe.
    #[serde(default, skip_serializing_if = "Option::is_none")]
